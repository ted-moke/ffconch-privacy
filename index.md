...
- **Anonymous usage analytics** sends a random identifier, the extension's
  **version number**, and a record of **which kinds of page you viewed and
  which Conch features you used** to Google Analytics, so the developer can
  tell which parts of the extension are worth the work and which are broken.
  It describes the *kind* of page ("matchup", "standings") and the *name* of a
  feature — never any league, roster, or account data, and never a URL.
  Switchable off in the popup.
...
So the developer can tell which extension versions are in use, which parts of
the extension people actually use, and which parts are silently broken, the
extension sends a small amount of anonymous usage data to **Google Analytics**
(which processes it on the developer's behalf).

Everything below describes Conch FF's own surfaces and the *type* of Sleeper
page they appear on. None of it identifies you, your leagues, or anything you
are looking at.

The examples given are illustrative rather than a complete list: as features
are added or retired, the names in these categories change, but what is
collected does not. The guarantee is not the list of names — it is that every
value sent is a fixed word chosen in advance by the developer and built into
the extension, so a category below can never quietly start carrying something
about you.

- **What is sent — an anonymous identity:** a random identifier generated by
  the extension (not derived from your Sleeper account, your browser profile,
  or any personal data), the extension's version number, and install/update
  events.
- **What is sent — which kinds of page, and for how long:** the **page type**
  you viewed (a fixed word from a short list the extension is built with, such
  as "matchup", "standings" or "scores"), whether Sleeper served you its
  classic or redesigned site, and **how long that page was visible**. This
  tells the developer which pages are worth improving. It is the page's type
  only: never its address, never the league or team it belongs to, and never
  anything shown on it. If Sleeper's redesign reaches a page the extension
  doesn't support yet, the same page type is reported so the developer knows
  to add support.
- **What is sent — which Conch features you used:** the **name of a Conch FF
  feature and the action taken on it** — for example that a trade grid was
  opened, that a hover card was shown, or that an announcement from the
  developer was dismissed. These name the extension's own controls, never
  Sleeper's, and carry no information about what the feature displayed.
- **What is sent — your Conch FF settings:** when you switch one of the
  extension's own features on or off, the **setting's name and its new value**
  are sent, plus a once-daily summary of which features you have switched off.
  This is how the developer learns that a feature is unpopular rather than
  merely unused. These are the extension's own settings — nothing from
  Sleeper's settings, and nothing about your account.
- **What is sent — clicks on the developer's own links:** when you follow a
  link from the extension to the Conch website or to the project's social
  account, the **fact of that click and which link it was**.
...
- **What is never sent:** anything about your Sleeper account, your leagues,
  your rosters, your matchups, the players you hold, page content, page
  addresses, or your browsing anywhere outside the extension's own surfaces.
  Analytics events contain no fantasy data at all. Every value is drawn from a
  fixed list built into the extension, and anything not on that list is
  discarded before it is sent rather than passed along.
...
- **`chrome.storage.local`** — a cached copy of the NFL player dictionary,
  ...
  housekeeping timestamps (last analytics send, last daily settings summary,
  last update check). ESPN
...
analytics (a random identifier, the extension version, the types of page
viewed, and which Conch FF features were used — never any fantasy data) is
processed by Google Analytics on the developer's behalf, exactly as described
above. Beyond that, the only party the extension communicates with
